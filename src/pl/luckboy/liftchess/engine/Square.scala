...
object Square
{
  ...
  /**
   * Składa pola ruchu pionka na danym polu.
   * @param sq 			pole.
   * @param side		strona.
   ...
   */
  def foldPawnMoveSquare[T](sq: Int, side: Side)(z: T)(p: (T, Int) => Boolean)(f: (T, Int) => T): T = throw new Exception
  ...
  /** Składa pola ruchu bierki na danym polu (z wyjątkiem pionków).
   * @param sq 			pole.
   * @param piece		bierka.
   * @param z			wartość początkowa.
   * @param p			funkcja przerwania (gdy false przerywa składanie dla aktualnej linii ale nie samo składanie).
